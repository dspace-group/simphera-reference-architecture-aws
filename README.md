...
| <a name="input_linuxExecutionCapacityType"></a> [linuxExecutionCapacityType](#input\_linuxExecutionCapacityType) | The capacity type of the Linux nodes to be used. Defaults to 'ON\_DEMAND' and can be changed to 'SPOT'. Be ware that using spot instances can result in abrupt termination of simulation/validation jobs and corresponding 'error' results. | `string` | `"ON_DEMAND"` | no |
