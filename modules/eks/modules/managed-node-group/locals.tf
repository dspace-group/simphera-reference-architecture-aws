locals {
  userdata_params = {
    ...
    pre_userdata           = local.node_group_config["pre_userdata"]         # Applied to all launch templates
    bootstrap_extra_args   = local.node_group_config["bootstrap_extra_args"] # used only when custom_ami_id specified e.g., bootstrap_extra_args="--use-max-pods false --container-runtime containerd"
    post_userdata          = local.node_group_config["post_userdata"]        # used only when custom_ami_id specified
    kubelet_extra_args     = local.node_group_config["kubelet_extra_args"]   # used only when custom_ami_id specified e.g., kubelet_extra_args="--node-labels=arch=x86,WorkerType=SPOT --max-pods=50 --register-with-taints=spot=true:NoSchedule"  # Equivalent to k8s_labels used in managed node groups
